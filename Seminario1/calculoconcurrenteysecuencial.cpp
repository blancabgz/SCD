...
//
// Ejercicio "Medir los tiempos del cálculo concurrente y secuencial de la integral"
...
double calcular_integral_concurrente( )
{
  ...
  future<double> futuros[n];  // iniciar el vector de futuros
  for(int i = 0; i < n; i++){
    futuros[i] = async(launch::async,funcion_hebra, i); // almacena los diferentes valores en el vector futuros
  }
  for(int j = 0; j < n; j++){
    suma_total += futuros[j].get(); // obtener, sumar y almacenar en el vector
  }
  return suma_total;
}
// -----------------------------------------------------------------------------
